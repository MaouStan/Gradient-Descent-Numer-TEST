...
np.random.seed(0)
X_linear = np.random.rand(10, 1) * 10
y_linear = 2 * X_linear.squeeze() + np.random.randn(10)  # Linear relationship with noise
...
np.random.seed(0)
X_logistic = np.random.rand(10, 2) * 10
...
X_neural, y_neural = make_classification(n_samples=10, n_features=5, n_classes=2, random_state=42)
